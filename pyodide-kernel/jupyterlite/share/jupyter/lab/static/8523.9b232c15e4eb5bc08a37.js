(self['webpackChunk_jupyterlab_application_top'] =
  self['webpackChunk_jupyterlab_application_top'] || []).push([
  [8523],
  {
    34647: (r, e, o) => {
      'use strict';
      o.d(e, { Z: () => a });
      var t = o(94015);
      var n = o.n(t);
      var i = o(23645);
      var c = o.n(i);
      var s = c()(n());
      s.push([
        r.id,
        '/*\n\n    Name:       Tomorrow Night - Eighties\n    Author:     Chris Kempson\n\n    CodeMirror template by Jan T. Sott (https://github.com/idleberg/base16-codemirror)\n    Original Base16 color scheme by Chris Kempson (https://github.com/chriskempson/base16)\n\n*/\n\n.cm-s-tomorrow-night-eighties.CodeMirror { background: #000000; color: #CCCCCC; }\n.cm-s-tomorrow-night-eighties div.CodeMirror-selected { background: #2D2D2D; }\n.cm-s-tomorrow-night-eighties .CodeMirror-line::selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span::selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span > span::selection { background: rgba(45, 45, 45, 0.99); }\n.cm-s-tomorrow-night-eighties .CodeMirror-line::-moz-selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span::-moz-selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span > span::-moz-selection { background: rgba(45, 45, 45, 0.99); }\n.cm-s-tomorrow-night-eighties .CodeMirror-gutters { background: #000000; border-right: 0px; }\n.cm-s-tomorrow-night-eighties .CodeMirror-guttermarker { color: #f2777a; }\n.cm-s-tomorrow-night-eighties .CodeMirror-guttermarker-subtle { color: #777; }\n.cm-s-tomorrow-night-eighties .CodeMirror-linenumber { color: #515151; }\n.cm-s-tomorrow-night-eighties .CodeMirror-cursor { border-left: 1px solid #6A6A6A; }\n\n.cm-s-tomorrow-night-eighties span.cm-comment { color: #d27b53; }\n.cm-s-tomorrow-night-eighties span.cm-atom { color: #a16a94; }\n.cm-s-tomorrow-night-eighties span.cm-number { color: #a16a94; }\n\n.cm-s-tomorrow-night-eighties span.cm-property, .cm-s-tomorrow-night-eighties span.cm-attribute { color: #99cc99; }\n.cm-s-tomorrow-night-eighties span.cm-keyword { color: #f2777a; }\n.cm-s-tomorrow-night-eighties span.cm-string { color: #ffcc66; }\n\n.cm-s-tomorrow-night-eighties span.cm-variable { color: #99cc99; }\n.cm-s-tomorrow-night-eighties span.cm-variable-2 { color: #6699cc; }\n.cm-s-tomorrow-night-eighties span.cm-def { color: #f99157; }\n.cm-s-tomorrow-night-eighties span.cm-bracket { color: #CCCCCC; }\n.cm-s-tomorrow-night-eighties span.cm-tag { color: #f2777a; }\n.cm-s-tomorrow-night-eighties span.cm-link { color: #a16a94; }\n.cm-s-tomorrow-night-eighties span.cm-error { background: #f2777a; color: #6A6A6A; }\n\n.cm-s-tomorrow-night-eighties .CodeMirror-activeline-background { background: #343600; }\n.cm-s-tomorrow-night-eighties .CodeMirror-matchingbracket { text-decoration: underline; color: white !important; }\n',
        '',
        {
          version: 3,
          sources: [
            'webpack://./node_modules/codemirror/theme/tomorrow-night-eighties.css',
          ],
          names: [],
          mappings:
            'AAAA;;;;;;;;CAQC;;AAED,2CAA2C,mBAAmB,EAAE,cAAc,EAAE;AAChF,wDAAwD,mBAAmB,EAAE;AAC7E,uMAAuM,kCAAkC,EAAE;AAC3O,sNAAsN,kCAAkC,EAAE;AAC1P,oDAAoD,mBAAmB,EAAE,iBAAiB,EAAE;AAC5F,yDAAyD,cAAc,EAAE;AACzE,gEAAgE,WAAW,EAAE;AAC7E,uDAAuD,cAAc,EAAE;AACvE,mDAAmD,8BAA8B,EAAE;;AAEnF,gDAAgD,cAAc,EAAE;AAChE,6CAA6C,cAAc,EAAE;AAC7D,+CAA+C,cAAc,EAAE;;AAE/D,kGAAkG,cAAc,EAAE;AAClH,gDAAgD,cAAc,EAAE;AAChE,+CAA+C,cAAc,EAAE;;AAE/D,iDAAiD,cAAc,EAAE;AACjE,mDAAmD,cAAc,EAAE;AACnE,4CAA4C,cAAc,EAAE;AAC5D,gDAAgD,cAAc,EAAE;AAChE,4CAA4C,cAAc,EAAE;AAC5D,6CAA6C,cAAc,EAAE;AAC7D,8CAA8C,mBAAmB,EAAE,cAAc,EAAE;;AAEnF,kEAAkE,mBAAmB,EAAE;AACvF,4DAA4D,0BAA0B,EAAE,uBAAuB,EAAE',
          sourcesContent: [
            '/*\n\n    Name:       Tomorrow Night - Eighties\n    Author:     Chris Kempson\n\n    CodeMirror template by Jan T. Sott (https://github.com/idleberg/base16-codemirror)\n    Original Base16 color scheme by Chris Kempson (https://github.com/chriskempson/base16)\n\n*/\n\n.cm-s-tomorrow-night-eighties.CodeMirror { background: #000000; color: #CCCCCC; }\n.cm-s-tomorrow-night-eighties div.CodeMirror-selected { background: #2D2D2D; }\n.cm-s-tomorrow-night-eighties .CodeMirror-line::selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span::selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span > span::selection { background: rgba(45, 45, 45, 0.99); }\n.cm-s-tomorrow-night-eighties .CodeMirror-line::-moz-selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span::-moz-selection, .cm-s-tomorrow-night-eighties .CodeMirror-line > span > span::-moz-selection { background: rgba(45, 45, 45, 0.99); }\n.cm-s-tomorrow-night-eighties .CodeMirror-gutters { background: #000000; border-right: 0px; }\n.cm-s-tomorrow-night-eighties .CodeMirror-guttermarker { color: #f2777a; }\n.cm-s-tomorrow-night-eighties .CodeMirror-guttermarker-subtle { color: #777; }\n.cm-s-tomorrow-night-eighties .CodeMirror-linenumber { color: #515151; }\n.cm-s-tomorrow-night-eighties .CodeMirror-cursor { border-left: 1px solid #6A6A6A; }\n\n.cm-s-tomorrow-night-eighties span.cm-comment { color: #d27b53; }\n.cm-s-tomorrow-night-eighties span.cm-atom { color: #a16a94; }\n.cm-s-tomorrow-night-eighties span.cm-number { color: #a16a94; }\n\n.cm-s-tomorrow-night-eighties span.cm-property, .cm-s-tomorrow-night-eighties span.cm-attribute { color: #99cc99; }\n.cm-s-tomorrow-night-eighties span.cm-keyword { color: #f2777a; }\n.cm-s-tomorrow-night-eighties span.cm-string { color: #ffcc66; }\n\n.cm-s-tomorrow-night-eighties span.cm-variable { color: #99cc99; }\n.cm-s-tomorrow-night-eighties span.cm-variable-2 { color: #6699cc; }\n.cm-s-tomorrow-night-eighties span.cm-def { color: #f99157; }\n.cm-s-tomorrow-night-eighties span.cm-bracket { color: #CCCCCC; }\n.cm-s-tomorrow-night-eighties span.cm-tag { color: #f2777a; }\n.cm-s-tomorrow-night-eighties span.cm-link { color: #a16a94; }\n.cm-s-tomorrow-night-eighties span.cm-error { background: #f2777a; color: #6A6A6A; }\n\n.cm-s-tomorrow-night-eighties .CodeMirror-activeline-background { background: #343600; }\n.cm-s-tomorrow-night-eighties .CodeMirror-matchingbracket { text-decoration: underline; color: white !important; }\n',
          ],
          sourceRoot: '',
        },
      ]);
      const a = s;
    },
    23645: (r) => {
      'use strict';
      r.exports = function (r) {
        var e = [];
        e.toString = function e() {
          return this.map(function (e) {
            var o = r(e);
            if (e[2]) {
              return '@media '.concat(e[2], ' {').concat(o, '}');
            }
            return o;
          }).join('');
        };
        e.i = function (r, o, t) {
          if (typeof r === 'string') {
            r = [[null, r, '']];
          }
          var n = {};
          if (t) {
            for (var i = 0; i < this.length; i++) {
              var c = this[i][0];
              if (c != null) {
                n[c] = true;
              }
            }
          }
          for (var s = 0; s < r.length; s++) {
            var a = [].concat(r[s]);
            if (t && n[a[0]]) {
              continue;
            }
            if (o) {
              if (!a[2]) {
                a[2] = o;
              } else {
                a[2] = ''.concat(o, ' and ').concat(a[2]);
              }
            }
            e.push(a);
          }
        };
        return e;
      };
    },
    94015: (r) => {
      'use strict';
      function e(r, e) {
        return c(r) || i(r, e) || t(r, e) || o();
      }
      function o() {
        throw new TypeError(
          'Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.'
        );
      }
      function t(r, e) {
        if (!r) return;
        if (typeof r === 'string') return n(r, e);
        var o = Object.prototype.toString.call(r).slice(8, -1);
        if (o === 'Object' && r.constructor) o = r.constructor.name;
        if (o === 'Map' || o === 'Set') return Array.from(r);
        if (o === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(o))
          return n(r, e);
      }
      function n(r, e) {
        if (e == null || e > r.length) e = r.length;
        for (var o = 0, t = new Array(e); o < e; o++) {
          t[o] = r[o];
        }
        return t;
      }
      function i(r, e) {
        var o =
          r &&
          ((typeof Symbol !== 'undefined' && r[Symbol.iterator]) || r['@@iterator']);
        if (o == null) return;
        var t = [];
        var n = true;
        var i = false;
        var c, s;
        try {
          for (o = o.call(r); !(n = (c = o.next()).done); n = true) {
            t.push(c.value);
            if (e && t.length === e) break;
          }
        } catch (a) {
          i = true;
          s = a;
        } finally {
          try {
            if (!n && o['return'] != null) o['return']();
          } finally {
            if (i) throw s;
          }
        }
        return t;
      }
      function c(r) {
        if (Array.isArray(r)) return r;
      }
      r.exports = function r(o) {
        var t = e(o, 4),
          n = t[1],
          i = t[3];
        if (typeof btoa === 'function') {
          var c = btoa(unescape(encodeURIComponent(JSON.stringify(i))));
          var s = 'sourceMappingURL=data:application/json;charset=utf-8;base64,'.concat(
            c
          );
          var a = '/*# '.concat(s, ' */');
          var m = i.sources.map(function (r) {
            return '/*# sourceURL='.concat(i.sourceRoot || '').concat(r, ' */');
          });
          return [n].concat(m).concat([a]).join('\n');
        }
        return [n].join('\n');
      };
    },
    48523: (r, e, o) => {
      'use strict';
      o.r(e);
      o.d(e, { default: () => a });
      var t = o(93379);
      var n = o.n(t);
      var i = o(34647);
      var c = {};
      c.insert = 'head';
      c.singleton = false;
      var s = n()(i.Z, c);
      const a = i.Z.locals || {};
    },
    93379: (r, e, o) => {
      'use strict';
      var t = (function r() {
        var e;
        return function r() {
          if (typeof e === 'undefined') {
            e = Boolean(window && document && document.all && !window.atob);
          }
          return e;
        };
      })();
      var n = (function r() {
        var e = {};
        return function r(o) {
          if (typeof e[o] === 'undefined') {
            var t = document.querySelector(o);
            if (window.HTMLIFrameElement && t instanceof window.HTMLIFrameElement) {
              try {
                t = t.contentDocument.head;
              } catch (n) {
                t = null;
              }
            }
            e[o] = t;
          }
          return e[o];
        };
      })();
      var i = [];
      function c(r) {
        var e = -1;
        for (var o = 0; o < i.length; o++) {
          if (i[o].identifier === r) {
            e = o;
            break;
          }
        }
        return e;
      }
      function s(r, e) {
        var o = {};
        var t = [];
        for (var n = 0; n < r.length; n++) {
          var s = r[n];
          var a = e.base ? s[0] + e.base : s[0];
          var m = o[a] || 0;
          var A = ''.concat(a, ' ').concat(m);
          o[a] = m + 1;
          var h = c(A);
          var l = { css: s[1], media: s[2], sourceMap: s[3] };
          if (h !== -1) {
            i[h].references++;
            i[h].updater(l);
          } else {
            i.push({ identifier: A, updater: d(l, e), references: 1 });
          }
          t.push(A);
        }
        return t;
      }
      function a(r) {
        var e = document.createElement('style');
        var t = r.attributes || {};
        if (typeof t.nonce === 'undefined') {
          var i = true ? o.nc : 0;
          if (i) {
            t.nonce = i;
          }
        }
        Object.keys(t).forEach(function (r) {
          e.setAttribute(r, t[r]);
        });
        if (typeof r.insert === 'function') {
          r.insert(e);
        } else {
          var c = n(r.insert || 'head');
          if (!c) {
            throw new Error(
              "Couldn't find a style target. This probably means that the value for the 'insert' parameter is invalid."
            );
          }
          c.appendChild(e);
        }
        return e;
      }
      function m(r) {
        if (r.parentNode === null) {
          return false;
        }
        r.parentNode.removeChild(r);
      }
      var A = (function r() {
        var e = [];
        return function r(o, t) {
          e[o] = t;
          return e.filter(Boolean).join('\n');
        };
      })();
      function h(r, e, o, t) {
        var n = o
          ? ''
          : t.media
          ? '@media '.concat(t.media, ' {').concat(t.css, '}')
          : t.css;
        if (r.styleSheet) {
          r.styleSheet.cssText = A(e, n);
        } else {
          var i = document.createTextNode(n);
          var c = r.childNodes;
          if (c[e]) {
            r.removeChild(c[e]);
          }
          if (c.length) {
            r.insertBefore(i, c[e]);
          } else {
            r.appendChild(i);
          }
        }
      }
      function l(r, e, o) {
        var t = o.css;
        var n = o.media;
        var i = o.sourceMap;
        if (n) {
          r.setAttribute('media', n);
        } else {
          r.removeAttribute('media');
        }
        if (i && typeof btoa !== 'undefined') {
          t += '\n/*# sourceMappingURL=data:application/json;base64,'.concat(
            btoa(unescape(encodeURIComponent(JSON.stringify(i)))),
            ' */'
          );
        }
        if (r.styleSheet) {
          r.styleSheet.cssText = t;
        } else {
          while (r.firstChild) {
            r.removeChild(r.firstChild);
          }
          r.appendChild(document.createTextNode(t));
        }
      }
      var g = null;
      var u = 0;
      function d(r, e) {
        var o;
        var t;
        var n;
        if (e.singleton) {
          var i = u++;
          o = g || (g = a(e));
          t = h.bind(null, o, i, false);
          n = h.bind(null, o, i, true);
        } else {
          o = a(e);
          t = l.bind(null, o, e);
          n = function r() {
            m(o);
          };
        }
        t(r);
        return function e(o) {
          if (o) {
            if (o.css === r.css && o.media === r.media && o.sourceMap === r.sourceMap) {
              return;
            }
            t((r = o));
          } else {
            n();
          }
        };
      }
      r.exports = function (r, e) {
        e = e || {};
        if (!e.singleton && typeof e.singleton !== 'boolean') {
          e.singleton = t();
        }
        r = r || [];
        var o = s(r, e);
        return function r(t) {
          t = t || [];
          if (Object.prototype.toString.call(t) !== '[object Array]') {
            return;
          }
          for (var n = 0; n < o.length; n++) {
            var a = o[n];
            var m = c(a);
            i[m].references--;
          }
          var A = s(t, e);
          for (var h = 0; h < o.length; h++) {
            var l = o[h];
            var g = c(l);
            if (i[g].references === 0) {
              i[g].updater();
              i.splice(g, 1);
            }
          }
          o = A;
        };
      };
    },
  },
]);
//# sourceMappingURL=8523.9b232c15e4eb5bc08a37.js.map?v=9b232c15e4eb5bc08a37
