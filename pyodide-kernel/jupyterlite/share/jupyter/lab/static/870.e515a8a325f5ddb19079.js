(self['webpackChunk_jupyterlab_application_top'] =
  self['webpackChunk_jupyterlab_application_top'] || []).push([
  [870],
  {
    49425: (n, e, r) => {
      'use strict';
      r.d(e, { Z: () => s });
      var c = r(94015);
      var o = r.n(c);
      var t = r(23645);
      var a = r.n(t);
      var i = a()(o());
      i.push([
        n.id,
        '/*\n\n    Name:       oceanic-next\n    Author:     Filype Pereira (https://github.com/fpereira1)\n\n    Original oceanic-next color scheme by Dmitri Voronianski (https://github.com/voronianski/oceanic-next-color-scheme)\n\n*/\n\n.cm-s-oceanic-next.CodeMirror { background: #304148; color: #f8f8f2; }\n.cm-s-oceanic-next div.CodeMirror-selected { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-line::selection, .cm-s-oceanic-next .CodeMirror-line > span::selection, .cm-s-oceanic-next .CodeMirror-line > span > span::selection { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-line::-moz-selection, .cm-s-oceanic-next .CodeMirror-line > span::-moz-selection, .cm-s-oceanic-next .CodeMirror-line > span > span::-moz-selection { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-gutters { background: #304148; border-right: 10px; }\n.cm-s-oceanic-next .CodeMirror-guttermarker { color: white; }\n.cm-s-oceanic-next .CodeMirror-guttermarker-subtle { color: #d0d0d0; }\n.cm-s-oceanic-next .CodeMirror-linenumber { color: #d0d0d0; }\n.cm-s-oceanic-next .CodeMirror-cursor { border-left: 1px solid #f8f8f0; }\n\n.cm-s-oceanic-next span.cm-comment { color: #65737E; }\n.cm-s-oceanic-next span.cm-atom { color: #C594C5; }\n.cm-s-oceanic-next span.cm-number { color: #F99157; }\n\n.cm-s-oceanic-next span.cm-property { color: #99C794; }\n.cm-s-oceanic-next span.cm-attribute,\n.cm-s-oceanic-next span.cm-keyword { color: #C594C5; }\n.cm-s-oceanic-next span.cm-builtin { color: #66d9ef; }\n.cm-s-oceanic-next span.cm-string { color: #99C794; }\n\n.cm-s-oceanic-next span.cm-variable,\n.cm-s-oceanic-next span.cm-variable-2,\n.cm-s-oceanic-next span.cm-variable-3 { color: #f8f8f2; }\n.cm-s-oceanic-next span.cm-def { color: #6699CC; }\n.cm-s-oceanic-next span.cm-bracket { color: #5FB3B3; }\n.cm-s-oceanic-next span.cm-tag { color: #C594C5; }\n.cm-s-oceanic-next span.cm-header { color: #C594C5; }\n.cm-s-oceanic-next span.cm-link { color: #C594C5; }\n.cm-s-oceanic-next span.cm-error { background: #C594C5; color: #f8f8f0; }\n\n.cm-s-oceanic-next .CodeMirror-activeline-background { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-matchingbracket {\n  text-decoration: underline;\n  color: white !important;\n}\n',
        '',
        {
          version: 3,
          sources: ['webpack://./node_modules/codemirror/theme/oceanic-next.css'],
          names: [],
          mappings:
            'AAAA;;;;;;;CAOC;;AAED,gCAAgC,mBAAmB,EAAE,cAAc,EAAE;AACrE,6CAA6C,qCAAqC,EAAE;AACpF,sKAAsK,qCAAqC,EAAE;AAC7M,qLAAqL,qCAAqC,EAAE;AAC5N,yCAAyC,mBAAmB,EAAE,kBAAkB,EAAE;AAClF,8CAA8C,YAAY,EAAE;AAC5D,qDAAqD,cAAc,EAAE;AACrE,4CAA4C,cAAc,EAAE;AAC5D,wCAAwC,8BAA8B,EAAE;;AAExE,qCAAqC,cAAc,EAAE;AACrD,kCAAkC,cAAc,EAAE;AAClD,oCAAoC,cAAc,EAAE;;AAEpD,sCAAsC,cAAc,EAAE;AACtD;qCACqC,cAAc,EAAE;AACrD,qCAAqC,cAAc,EAAE;AACrD,oCAAoC,cAAc,EAAE;;AAEpD;;wCAEwC,cAAc,EAAE;AACxD,iCAAiC,cAAc,EAAE;AACjD,qCAAqC,cAAc,EAAE;AACrD,iCAAiC,cAAc,EAAE;AACjD,oCAAoC,cAAc,EAAE;AACpD,kCAAkC,cAAc,EAAE;AAClD,mCAAmC,mBAAmB,EAAE,cAAc,EAAE;;AAExE,uDAAuD,qCAAqC,EAAE;AAC9F;EACE,0BAA0B;EAC1B,uBAAuB;AACzB',
          sourcesContent: [
            '/*\n\n    Name:       oceanic-next\n    Author:     Filype Pereira (https://github.com/fpereira1)\n\n    Original oceanic-next color scheme by Dmitri Voronianski (https://github.com/voronianski/oceanic-next-color-scheme)\n\n*/\n\n.cm-s-oceanic-next.CodeMirror { background: #304148; color: #f8f8f2; }\n.cm-s-oceanic-next div.CodeMirror-selected { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-line::selection, .cm-s-oceanic-next .CodeMirror-line > span::selection, .cm-s-oceanic-next .CodeMirror-line > span > span::selection { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-line::-moz-selection, .cm-s-oceanic-next .CodeMirror-line > span::-moz-selection, .cm-s-oceanic-next .CodeMirror-line > span > span::-moz-selection { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-gutters { background: #304148; border-right: 10px; }\n.cm-s-oceanic-next .CodeMirror-guttermarker { color: white; }\n.cm-s-oceanic-next .CodeMirror-guttermarker-subtle { color: #d0d0d0; }\n.cm-s-oceanic-next .CodeMirror-linenumber { color: #d0d0d0; }\n.cm-s-oceanic-next .CodeMirror-cursor { border-left: 1px solid #f8f8f0; }\n\n.cm-s-oceanic-next span.cm-comment { color: #65737E; }\n.cm-s-oceanic-next span.cm-atom { color: #C594C5; }\n.cm-s-oceanic-next span.cm-number { color: #F99157; }\n\n.cm-s-oceanic-next span.cm-property { color: #99C794; }\n.cm-s-oceanic-next span.cm-attribute,\n.cm-s-oceanic-next span.cm-keyword { color: #C594C5; }\n.cm-s-oceanic-next span.cm-builtin { color: #66d9ef; }\n.cm-s-oceanic-next span.cm-string { color: #99C794; }\n\n.cm-s-oceanic-next span.cm-variable,\n.cm-s-oceanic-next span.cm-variable-2,\n.cm-s-oceanic-next span.cm-variable-3 { color: #f8f8f2; }\n.cm-s-oceanic-next span.cm-def { color: #6699CC; }\n.cm-s-oceanic-next span.cm-bracket { color: #5FB3B3; }\n.cm-s-oceanic-next span.cm-tag { color: #C594C5; }\n.cm-s-oceanic-next span.cm-header { color: #C594C5; }\n.cm-s-oceanic-next span.cm-link { color: #C594C5; }\n.cm-s-oceanic-next span.cm-error { background: #C594C5; color: #f8f8f0; }\n\n.cm-s-oceanic-next .CodeMirror-activeline-background { background: rgba(101, 115, 126, 0.33); }\n.cm-s-oceanic-next .CodeMirror-matchingbracket {\n  text-decoration: underline;\n  color: white !important;\n}\n',
          ],
          sourceRoot: '',
        },
      ]);
      const s = i;
    },
    23645: (n) => {
      'use strict';
      n.exports = function (n) {
        var e = [];
        e.toString = function e() {
          return this.map(function (e) {
            var r = n(e);
            if (e[2]) {
              return '@media '.concat(e[2], ' {').concat(r, '}');
            }
            return r;
          }).join('');
        };
        e.i = function (n, r, c) {
          if (typeof n === 'string') {
            n = [[null, n, '']];
          }
          var o = {};
          if (c) {
            for (var t = 0; t < this.length; t++) {
              var a = this[t][0];
              if (a != null) {
                o[a] = true;
              }
            }
          }
          for (var i = 0; i < n.length; i++) {
            var s = [].concat(n[i]);
            if (c && o[s[0]]) {
              continue;
            }
            if (r) {
              if (!s[2]) {
                s[2] = r;
              } else {
                s[2] = ''.concat(r, ' and ').concat(s[2]);
              }
            }
            e.push(s);
          }
        };
        return e;
      };
    },
    94015: (n) => {
      'use strict';
      function e(n, e) {
        return a(n) || t(n, e) || c(n, e) || r();
      }
      function r() {
        throw new TypeError(
          'Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.'
        );
      }
      function c(n, e) {
        if (!n) return;
        if (typeof n === 'string') return o(n, e);
        var r = Object.prototype.toString.call(n).slice(8, -1);
        if (r === 'Object' && n.constructor) r = n.constructor.name;
        if (r === 'Map' || r === 'Set') return Array.from(n);
        if (r === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r))
          return o(n, e);
      }
      function o(n, e) {
        if (e == null || e > n.length) e = n.length;
        for (var r = 0, c = new Array(e); r < e; r++) {
          c[r] = n[r];
        }
        return c;
      }
      function t(n, e) {
        var r =
          n &&
          ((typeof Symbol !== 'undefined' && n[Symbol.iterator]) || n['@@iterator']);
        if (r == null) return;
        var c = [];
        var o = true;
        var t = false;
        var a, i;
        try {
          for (r = r.call(n); !(o = (a = r.next()).done); o = true) {
            c.push(a.value);
            if (e && c.length === e) break;
          }
        } catch (s) {
          t = true;
          i = s;
        } finally {
          try {
            if (!o && r['return'] != null) r['return']();
          } finally {
            if (t) throw i;
          }
        }
        return c;
      }
      function a(n) {
        if (Array.isArray(n)) return n;
      }
      n.exports = function n(r) {
        var c = e(r, 4),
          o = c[1],
          t = c[3];
        if (typeof btoa === 'function') {
          var a = btoa(unescape(encodeURIComponent(JSON.stringify(t))));
          var i = 'sourceMappingURL=data:application/json;charset=utf-8;base64,'.concat(
            a
          );
          var s = '/*# '.concat(i, ' */');
          var A = t.sources.map(function (n) {
            return '/*# sourceURL='.concat(t.sourceRoot || '').concat(n, ' */');
          });
          return [o].concat(A).concat([s]).join('\n');
        }
        return [o].join('\n');
      };
    },
    70870: (n, e, r) => {
      'use strict';
      r.r(e);
      r.d(e, { default: () => s });
      var c = r(93379);
      var o = r.n(c);
      var t = r(49425);
      var a = {};
      a.insert = 'head';
      a.singleton = false;
      var i = o()(t.Z, a);
      const s = t.Z.locals || {};
    },
    93379: (n, e, r) => {
      'use strict';
      var c = (function n() {
        var e;
        return function n() {
          if (typeof e === 'undefined') {
            e = Boolean(window && document && document.all && !window.atob);
          }
          return e;
        };
      })();
      var o = (function n() {
        var e = {};
        return function n(r) {
          if (typeof e[r] === 'undefined') {
            var c = document.querySelector(r);
            if (window.HTMLIFrameElement && c instanceof window.HTMLIFrameElement) {
              try {
                c = c.contentDocument.head;
              } catch (o) {
                c = null;
              }
            }
            e[r] = c;
          }
          return e[r];
        };
      })();
      var t = [];
      function a(n) {
        var e = -1;
        for (var r = 0; r < t.length; r++) {
          if (t[r].identifier === n) {
            e = r;
            break;
          }
        }
        return e;
      }
      function i(n, e) {
        var r = {};
        var c = [];
        for (var o = 0; o < n.length; o++) {
          var i = n[o];
          var s = e.base ? i[0] + e.base : i[0];
          var A = r[s] || 0;
          var l = ''.concat(s, ' ').concat(A);
          r[s] = A + 1;
          var m = a(l);
          var u = { css: i[1], media: i[2], sourceMap: i[3] };
          if (m !== -1) {
            t[m].references++;
            t[m].updater(u);
          } else {
            t.push({ identifier: l, updater: C(u, e), references: 1 });
          }
          c.push(l);
        }
        return c;
      }
      function s(n) {
        var e = document.createElement('style');
        var c = n.attributes || {};
        if (typeof c.nonce === 'undefined') {
          var t = true ? r.nc : 0;
          if (t) {
            c.nonce = t;
          }
        }
        Object.keys(c).forEach(function (n) {
          e.setAttribute(n, c[n]);
        });
        if (typeof n.insert === 'function') {
          n.insert(e);
        } else {
          var a = o(n.insert || 'head');
          if (!a) {
            throw new Error(
              "Couldn't find a style target. This probably means that the value for the 'insert' parameter is invalid."
            );
          }
          a.appendChild(e);
        }
        return e;
      }
      function A(n) {
        if (n.parentNode === null) {
          return false;
        }
        n.parentNode.removeChild(n);
      }
      var l = (function n() {
        var e = [];
        return function n(r, c) {
          e[r] = c;
          return e.filter(Boolean).join('\n');
        };
      })();
      function m(n, e, r, c) {
        var o = r
          ? ''
          : c.media
          ? '@media '.concat(c.media, ' {').concat(c.css, '}')
          : c.css;
        if (n.styleSheet) {
          n.styleSheet.cssText = l(e, o);
        } else {
          var t = document.createTextNode(o);
          var a = n.childNodes;
          if (a[e]) {
            n.removeChild(a[e]);
          }
          if (a.length) {
            n.insertBefore(t, a[e]);
          } else {
            n.appendChild(t);
          }
        }
      }
      function u(n, e, r) {
        var c = r.css;
        var o = r.media;
        var t = r.sourceMap;
        if (o) {
          n.setAttribute('media', o);
        } else {
          n.removeAttribute('media');
        }
        if (t && typeof btoa !== 'undefined') {
          c += '\n/*# sourceMappingURL=data:application/json;base64,'.concat(
            btoa(unescape(encodeURIComponent(JSON.stringify(t)))),
            ' */'
          );
        }
        if (n.styleSheet) {
          n.styleSheet.cssText = c;
        } else {
          while (n.firstChild) {
            n.removeChild(n.firstChild);
          }
          n.appendChild(document.createTextNode(c));
        }
      }
      var d = null;
      var f = 0;
      function C(n, e) {
        var r;
        var c;
        var o;
        if (e.singleton) {
          var t = f++;
          r = d || (d = s(e));
          c = m.bind(null, r, t, false);
          o = m.bind(null, r, t, true);
        } else {
          r = s(e);
          c = u.bind(null, r, e);
          o = function n() {
            A(r);
          };
        }
        c(n);
        return function e(r) {
          if (r) {
            if (r.css === n.css && r.media === n.media && r.sourceMap === n.sourceMap) {
              return;
            }
            c((n = r));
          } else {
            o();
          }
        };
      }
      n.exports = function (n, e) {
        e = e || {};
        if (!e.singleton && typeof e.singleton !== 'boolean') {
          e.singleton = c();
        }
        n = n || [];
        var r = i(n, e);
        return function n(c) {
          c = c || [];
          if (Object.prototype.toString.call(c) !== '[object Array]') {
            return;
          }
          for (var o = 0; o < r.length; o++) {
            var s = r[o];
            var A = a(s);
            t[A].references--;
          }
          var l = i(c, e);
          for (var m = 0; m < r.length; m++) {
            var u = r[m];
            var d = a(u);
            if (t[d].references === 0) {
              t[d].updater();
              t.splice(d, 1);
            }
          }
          r = l;
        };
      };
    },
  },
]);
//# sourceMappingURL=870.e515a8a325f5ddb19079.js.map?v=e515a8a325f5ddb19079
